...
namespace DomainTests
{
    public class FinancialTargetTests
    {
        ...
        [InlineData(0)]
        [InlineData(5)]
        public void AddTargetPoints_DoesNotThrow(int count)
        {
            FinancialTarget target = this.CreateFinancialTarget();
            IMyList list = new Mock<IMyList>().Object;
        ...
        [InlineData(1)]
        [InlineData(17)]
        public void AddTargetPoints_AddsSpecifiedNumberOfElementsToManualMock(int count)
        {
            FinancialTarget target = this.CreateFinancialTarget();
            ElementCountingList list = new ElementCountingList();
        ...
        [InlineData(216, 9, 71, 145)]
        [InlineData(216, 11, 71, 145)]
        public void AddTargetPoints_SpecificValueAtPositionInManualMock(int count, int month, int index, int expectedValue)
        {
            FinancialTarget target = this.CreateFinancialTarget(month);
            ElementValueTestingList list = new ElementValueTestingList(index);

            target.AddTargetPoints(list, count);

            Assert.Equal(expectedValue, list.ElementValue.Single());
        }
